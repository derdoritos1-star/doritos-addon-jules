...
public class NetheriteFinder extends Module {
    ...
    private final Setting<Integer> subzoneSize = sgTargeting.add(new IntSetting.Builder().name("subzone-size").description("Size of the mini-cube target").defaultValue(8).sliderRange(4, 16).build());
    private final Setting<Integer> explosionRadius = sgTargeting.add(new IntSetting.Builder().name("explosion-radius").description("Radius to track explosions around the target").defaultValue(12).sliderRange(1, 32).build());
    private final Setting<Integer> explosionWaitTicks = sgTargeting.add(new IntSetting.Builder().name("explosion-wait-ticks").description("Ticks to wait after explosion").defaultValue(5).sliderRange(1, 40).build());
    private final Setting<SettingColor> targetFillColor = sgTargeting.add(new ColorSetting.Builder().name("target-fill-color").defaultValue(new SettingColor(255, 255, 0, 100)).build());
    private final Setting<SettingColor> targetOutlineColor = sgTargeting.add(new ColorSetting.Builder().name("target-outline-color").defaultValue(new SettingColor(255, 255, 0, 255)).build());
    ...
    private volatile ChunkSectionPos activeTargetSection = null;
    private volatile Box activeSubzone = null;
    private final Queue<Box> pendingSubzones = new ConcurrentLinkedQueue<>();
    private int explosionWaitTimer = 0;
    ...
    private void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.packet instanceof BlockUpdateS2CPacket packet) {
            ...
            if (activeSubzone != null) {
                double ex = packet.center().getX();
                double ey = packet.center().getY();
                double ez = packet.center().getZ();
                double dx = Math.max(activeSubzone.minX - ex, Math.max(0, ex - activeSubzone.maxX));
                double dy = Math.max(activeSubzone.minY - ey, Math.max(0, ey - activeSubzone.maxY));
                double dz = Math.max(activeSubzone.minZ - ez, Math.max(0, ez - activeSubzone.maxZ));
                double distanceSq = dx * dx + dy * dy + dz * dz;

                double radius = explosionRadius.get();
                if (distanceSq <= radius * radius) {
                    explosionWaitTimer = explosionWaitTicks.get();
                }
            }
        }
    }
    ...
    private void updateTarget() {
        if (explosionWaitTimer > 0) {
            explosionWaitTimer--;
            if (explosionWaitTimer == 0) {
                boolean hasDebrisInActive = false;
                if (activeTargetSection != null) {
                    hasDebrisInActive = foundBlocks.stream().anyMatch(p -> ChunkSectionPos.from(p).equals(activeTargetSection));
                }

                if (!hasDebrisInActive) {
                    activeSubzone = pendingSubzones.poll();
                } else {
                    activeTargetSection = null;
                    activeSubzone = null;
                    pendingSubzones.clear();
        ...
        if (activeTargetSection != null && activeSubzone != null) {
            boolean hasDebrisInActive = foundBlocks.stream().anyMatch(p -> ChunkSectionPos.from(p).equals(activeTargetSection));
            if (hasDebrisInActive) {
                activeTargetSection = null;
                activeSubzone = null;
                pendingSubzones.clear();
        ...
        if (activeTargetSection != null && activeSubzone == null && pendingSubzones.isEmpty()) {
            boolean hasDebrisInActive = foundBlocks.stream().anyMatch(p -> ChunkSectionPos.from(p).equals(activeTargetSection));
            if (!hasDebrisInActive) {
                suspectedSet.remove(activeTargetSection);
            }
            activeTargetSection = null;
        }
        ...
        if (activeTargetSection == null) {
            for (ChunkSectionPos sec : suspectedSet) {
                if (suspectedQueue.contains(sec)) continue;

                boolean hasDebris = foundBlocks.stream().anyMatch(p -> ChunkSectionPos.from(p).equals(sec));
                if (!hasDebris) {
                    activeTargetSection = sec;
                    pendingSubzones.clear();

                    int size = subzoneSize.get();
                    for (int x = 0; x < 16; x += size) {
                        for (int y = 0; y < 16; y += size) {
                            for (int z = 0; z < 16; z += size) {
                                int sx = sec.getMinX() + x;
                                int sy = sec.getMinY() + y;
                                int sz = sec.getMinZ() + z;
                                Box subzone = new Box(sx, sy, sz,
                                        Math.min(sec.getMinX() + 16, sx + size),
                                        Math.min(sec.getMinY() + 16, sy + size),
                                        Math.min(sec.getMinZ() + 16, sz + size));
                                pendingSubzones.add(subzone);
                            }
                        }
                    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        suspectedSet.removeIf(sec -> {
            ...
            if (confirmedSections.contains(sec) && !hasBlocks) {
                ...
                if (activeTargetSection != null && activeTargetSection.equals(sec)) {
                    activeTargetSection = null;
                    activeSubzone = null;
                    pendingSubzones.clear();
                }
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (espSections.get()) {
            for (ChunkSectionPos sec : suspectedSet) {
                boolean isActive = activeTargetSection != null && activeTargetSection.equals(sec);

                if (isActive) {
                    event.renderer.box(sec.getMinX(), sec.getMinY(), sec.getMinZ(), sec.getMaxX() + 1, sec.getMaxY() + 1, sec.getMaxZ() + 1, sideColor.get(), lineColor.get(), ShapeMode.Both, 0);

                    if (activeSubzone != null) {
                        event.renderer.box(activeSubzone.minX, activeSubzone.minY, activeSubzone.minZ, activeSubzone.maxX, activeSubzone.maxY, activeSubzone.maxZ, targetFillColor.get(), targetOutlineColor.get(), ShapeMode.Both, 0);
                    }
                } else {
                    event.renderer.box(sec.getMinX(), sec.getMinY(), sec.getMinZ(), sec.getMaxX() + 1, sec.getMaxY() + 1, sec.getMaxZ() + 1, sideColor.get(), lineColor.get(), ShapeMode.Lines, 0);
                }
            }
        }
